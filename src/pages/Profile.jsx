...
function Profile() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await axios.put(`http://13.48.104.124:8081/admin/updateUser`, formDataToUpdate, {
        headers: {
  ...
  const handlePasswordChangeSubmit = async (event) => {
    ...
    try {
      const res = await axios.put('http://13.48.104.124:8081/admin/updatePassword', passwordUpdateRequest, {
        headers: {
          Authorization: `Bearer ${token}` // Replace with your actual token
        }
      });
