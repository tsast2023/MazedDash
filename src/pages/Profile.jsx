...
function Profile() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await axios.put(`http://192.168.0.112:8081/admin/updateUser`, formDataToUpdate, {
        headers: {
  ...
  const handlePasswordChangeSubmit = async (event) => {
    ...
    try {
      const res = await axios.put('http://192.168.0.112:8081/admin/updatePassword', passwordUpdateRequest, {
        headers: {
          Authorization: `Bearer ${token}` // Replace with your actual token
        }
      });
