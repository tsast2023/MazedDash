...
const Tutoriel = () => {
  ...
  const deleteItem = async (id) => {
    try {
      const res = await axios.delete(`http://192.168.0.102:8081/api/tuto/deleteTuto?id=${id}` , {headers : {Authorization: `Bearer ${token}`}});
  ...
  const addTuto = async (e) => {
    ...
    try {
        const res = await axios.post("http://192.168.0.102:8081/api/tuto/publishNow", formData, {
            headers: { 
