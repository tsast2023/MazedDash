...
function DetailEnchere(props) {
  ...
  const funModal = (user , enchere , encherissmentId) =>{
    ...
    const approverUser = async(enchereId , userId) =>{
      try {
        const res = await axios.post(`http://192.168.0.112:8081/api/bid/approve/${enchereId}/${userId}`, {} , {headers:{Authorization: `Bearer ${token}`}})
