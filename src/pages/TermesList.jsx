...
function TermesList() {
  ...
  const deleteItem = async(id) => {
    try {
      const res = await axios.delete(`http://13.48.104.124:8081/api/termes/deleteTerme/${id}` , {headers : {Authorization: `Bearer ${token}`}})
  ...
  const updateTermes = async(e , id)=>{
    e.preventDefault()
    try {
      const res = await axios.put(`http://13.48.104.124:8081/api/termes/updateTermme/${id}`, data , {headers : {Authorization: `Bearer ${token}`}});
      console.log(res.data)
    } catch (error) {
      console.log(error)
    }
