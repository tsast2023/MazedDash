...
getMe();
  ...
  useEffect(()=>{
    ...
    //pseudo=${userPseudo}&numTel=${userNumtel}&status=${userStatus}&page=${pageUser}
      try {
        const res = await axios.get(`http://192.168.0.112:8081/admin/users/filtredUser`, {headers : {Authorization: `Bearer ${token}`}})
  ...
  useEffect(()=>{
    const getAllDemandeCategories = async()=>{
      try {
        const res = await axios.get(`http://192.168.0.112:8081/api/demandes/filter?identifiant=${identifiantDemCat}&statusDemande=${statusDemCat}&page=${pageDemCat}&action=${actionDemCat}`, {headers : {Authorization: `Bearer ${token}`}})
